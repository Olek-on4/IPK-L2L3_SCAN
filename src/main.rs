...
impl Scanner {
    ...
    fn run(&self) -> Result<Vec<IpAddr>, ScannerError> {
        // Start timing the scan (currently unused)
...
/// Allows constructing a `Scanner` from `Cli` arguments, validating input and handling errors.
impl TryFrom<Cli> for Scanner {
    ...
    fn try_from(value: Cli) -> Result<Self, Self::Error> {
        ...
        match &value.interface { // May have -i
            Some(iface) => {
                match iface { // May have -i <value>
                    Some(iface) => { // Has interface, check availability
                        ...
                        match &value.subnets { // May have -s <value>...
                            Some(subnets) => { // Has subnet(s)
                                Ok(Self {
                                    interface: interface,
                                    networks: value.subnets.unwrap_or(vec![]),
                                    timeout: Duration::from_millis(value.timeout),
                                })
                            }

                            _ => { // Has no subnet range specified
                                return Err(ScannerError {
                                    code: ScannerExitCode::Cli,
            ...
            _ => { // Has no -i, no interface specified
                Err(ScannerError {
                    code: ScannerExitCode::Cli,
